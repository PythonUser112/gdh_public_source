...
export (bool) var retry_on_die = false
...
func change_level(level):
	var fm = Loader.bload("FinishMessgae.tscn")
	fm.connect("retry", self, "_on_Node2D_retry")
	fm.connect("main_menu", self, "_on_Node2D_main_menu")
	$L/Control.add_child(fm)
	attempt = 1
	_level = level
...
func custom_level():
	map.get_node("CustomSetup").init()
	map.get_node("CustomSetup").set_player(load("res://Player/Player.tscn").instance())
...
func _on_Player_dead():
	...
	if retry_on_die:
		...
		$L/Control/Node2D/VBoxContainer/Label.hide()
		$L/Control.show()

func retry():
	attempt += 1
	$L/Control/Node2D/VBoxContainer/Label.show()
	player.queue_free()
	map.queue_free()
	next()
